            ...
            <div class="bg-white mb-4 p-6 rounded-xl border shadow-md text-gray-700 break-words">
                <div class="flex items-start">
                    @if ($reply->sender->profile_picture)
                        @if ($reply->sender->name == Auth::user()->name)
                            <img src="{{ asset($reply->sender->profile_picture) }}" alt="{{ $reply->sender->name }}" class="h-12 rounded-full mr-4 border border-gray-400">
                        @else
                            <img src="{{ asset($reply->sender->profile_picture) }}" alt="{{ $reply->sender->name }}" class="h-12 rounded-full mr-4 border border-gray-400">
                        @endif
                    @else
